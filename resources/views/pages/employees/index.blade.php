...
<div class="filters mx-4 mb-3">
    <div class="filters-child">
        <button onclick="window.location.href='{{ route('employees.create') }}'" class="btn btn-primary" id="btn-add-record">{{ $add }}</button>
...
$(document).ready(function(){
    ...
    columnDefs = {
        ...
        cellRenderer: function(params){
            ...
            var edit_url = '{{ route("employees.edit", ":id") }}';
    ...
    for (var i = data.column.length - 1; i >= 0; i--) {
        ...
        if (data.column[i].field == "name") {
            data.column[i].cellRenderer = function display(params) {
                ...
                var middleName = params.data.middle_name;
                var lastName = params.data.last_name;

                return firstName.charAt(0).toUpperCase() + firstName.substr(1) + ' ' + middleName.charAt(0).toUpperCase() + '. ' + lastName.charAt(0).toUpperCase() + lastName.substr(1);
            }
        }

        // if (data.column[i].field == "birthdate") {
        //     data.column[i].cellRenderer = function display(params) {
        //         var bday = params.data.birthdate;
    ...
    var gridOptions = {
        ...
        rowStyle: { 
            fontFamily: ['Roboto', 'Poppins', 'Montserrat', 'sans-serif'],
            fontWeight: 'normal',
            fontSize: '1em',
            color: '#777'
        },
    ...
    $('#btn-remove').on('click', function(){
        var destroy = '{{ route("employees.destroy", ":id") }}';
