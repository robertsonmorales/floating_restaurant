...
<div class="filters mx-4 mb-3">
    <div class="filters-child">
        <button onclick="window.location.href='{{ route('order_status.create') }}'" class="btn btn-primary" id="btn-add-record">{{ $add }}</button>
...
$(document).ready(function(){
    ...
    columnDefs = {
        ...
        cellRenderer: function(params){
            ...
            var edit_url = '{{ route("order_status.edit", ":id") }}';
    ...
    for (var i = data.column.length - 1; i >= 0; i--) {
        if (data.column[i].field == "name") {
            data.column[i].cellRenderer = function display(params) {
                if (params.data.color == null || params.data.color == undefined || params.data.color == "") {
                    return '<div class="d-flex align-items-center">\
                            <div style="width:5px; height: 5px" class="bg-secondary rounded-circle mr-2"></div>\
                            ' + params.data.name + '\
                        <div>';
                }else if (params.data.color == '#ffffff') {
                    return '<div class="d-flex align-items-center">\
                            <div style="width:5px; height: 5px; color: #fff; background-color: #eee; color: '+params.data.color+';" class="rounded-circle mr-2"></div>\
                            ' + params.data.name + '\
                        <div>';
                }else{
                    return '<div class="d-flex align-items-center">\
                            <div style="width:5px; height: 5px; background-color: '+params.data.color +';" class="rounded-circle mr-2"></div>\
                            ' + params.data.name + '\
                        <div>';
                }
            }
    ...
    $('#btn-remove').on('click', function(){
        var destroy = '{{ route("order_status.destroy", ":id") }}';
